...
#Required data
# a) Site DEM
# b) Watershed Outlet Location
# c) Critical Juncture Locations
# d) stream network -- or rough area threshold

#Broadly, the outline for this script should look like: 
# 1) Setup workspace
# 2) Create function
# 3) Execute function for the Southeast

#Flow of functions should loook like 
# i) delineate watershed
# ii) create flow network (if needed?!)
# iii) STIC Locations
...
net_fun<-function(dem, shed=NA, threshold){

  #If watershed shape is provide, clip dem to watershed area for speed! 
  if(!is.na(shed)){
    dem<-raster::crop(dem, shed)
    dem<-raster::mask(dem, shed)
  }
...
stic_fun<-function(
  ...
  dem, #dem of area
  shed = NA, #watershed shape
  threshold, 
  hydro_pnts,
  streams #Vector file of streams
){
  ...
  #Create function to remove points along stream -------------------------------
  ...
  # Remove points associated with existing hydrologic infrastructure ------------
...
shed<-ws_fun(dem, pp,  scratch_dir)
stream<-net_fun(dem, shed, threshold=25000)
